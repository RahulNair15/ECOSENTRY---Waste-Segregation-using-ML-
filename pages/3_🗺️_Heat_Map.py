...
def calc_tensionfactor(bags):
    st.title("Garbage Heatmap")
    tfactor = bags / 100
    heatmap_data.append([lati,long,tfactor])
    folium.plugins.HeatMap(heatmap_data, min_opacity=0.2, radius=15, blur=10, max_zoom=13).add_to(m)
    st.success("Heat Map updated successfully!")
    return folium_static(m)

# Report Garbage
bags = st.slider('Number of Garbage Bags', 5, 100, 10)
st.button("Report Garbage", type="primary", on_click=calc_tensionfactor, args=(bags,))
...
# Create a Folium map
m = folium.Map(location=[19.029950620187304, 73.01651434889969], zoom_start=16)
...
folium.Marker(location=(lati, long), popup="You are here!").add_to(m)
